...
def vertical_win(board, position, player):
    ...
    for i in range(len(pot_rows)-2):
        ...
        if layout.count(player) >= 3:
            gui.visualize_winning_tokens( board, [(pot_rows[i], col), (pot_rows[i+1], col), (pot_rows[i+2], col), position])
            return True
    return False        
...
def horizantal_win(board, position, player):
    ...
    for i in range(len(pot_cols)-2 ):
        ...
        if layout.count(player) >= 3:
            gui.visualize_winning_tokens( board, [(row, pot_cols[i]), (row, pot_cols[i+1]), (row, pot_cols[i+2]), position])
            return True
    return False
...
def right_diag_win(board, position, player):
    ...
    for i in range(len(pot_coords)-2):
        ...
        if layout.count(player) >= 3:      
            gui.visualize_winning_tokens( board, [(pot_coords[i][0], pot_coords[i][1]), (pot_coords[i+1][0], pot_coords[i+1][1]), (pot_coords[i+2][0], pot_coords[i+2][1]), position])
            return True
    return False
