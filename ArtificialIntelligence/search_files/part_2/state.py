...
# These hdistances are the functions that must be altered!:

def hdistance(s): #This is uniform cost and not based on any heuristic # The uniform cost is already done. 
    return 0
...
def hdistance2(s): #This will be the Manhattan distance heuristic
    ...
    for i in range(len(s[0])):
        if i != s[0][i]:
            ...
            curr_column = i%n
            correct_col = s[0][i]%n
            counter += abs(curr_column-correct_col) + abs(curr_row-correct_row)
    return counter