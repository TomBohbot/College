...
def insert(h, s):
    ...
    while i>0 and val(f[i])<val(f[(i-1)//2]): # while item i's value is smaller than the value of his father, swap! # this while loop is just heapify:
...
def val(s):  # returns f(x) which is path len + heuristic distance from target
    return state.hdistance2(s)+state.path_len(s) #path_len = g and hdistance = heuristic # this line must be altered every run when we implement hdistance, hdistance1, and hdistance2
'''
