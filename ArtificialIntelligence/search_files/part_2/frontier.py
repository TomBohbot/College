...
def val(s):  # returns f(x) which is path len + heuristic distance from target
    return state.hdistance(s)+state.path_len(s) #path_len = g and hdistance = heuristic # this line must be altered every run when we implement hdistance, hdistance1, and hdistance2
'''
